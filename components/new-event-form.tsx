...
import { DATE_CERTAINTY_OPTIONS, type DateCertainty } from "@/lib/utils";
...
export function NewEventForm({ eventsList, parentEventId, parentEvent, allTopics, allCategories, allOrganizations, allVenues, orgLocationMappings }: NewEventFormProps) {
  const [state, formAction] = useFormState(createEvent, undefined);
  const [selectedTopicIds, setSelectedTopicIds] = useState<string[]>([]);
  const [selectedCategoryIds, setSelectedCategoryIds] = useState<string[]>([]);

  // Controlled state for organization and venue fields
  const [hostOrganizationId, setHostOrganizationId] = useState("");
  const [organizerOrganizationId, setOrganizerOrganizationId] = useState("");
  const [onlineHostOrganizationId, setOnlineHostOrganizationId] = useState("");
  const [venueId, setVenueId] = useState("");
  const [showVenuePicker, setShowVenuePicker] = useState(false);
  const [showSpaceOverride, setShowSpaceOverride] = useState(false);
        ...
        <div className="rounded-lg border p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <Input id="eventDateEnd" name="eventDateEnd" type="date" className="flex-1" />
                <select
                  value={endCertainty}
