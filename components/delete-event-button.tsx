...
export function DeleteEventButton({ id, eventName, sessionCount = 0 }: DeleteEventButtonProps) {
    ...
    startTransition(async () => {
      try {
        ...
        const response = await fetch(`/api/pipeline`, {
          ...
          body: JSON.stringify({
            endpoint: `/api/v1/admin/events/${id}${queryString ? `?${queryString}` : ""}`,
            method: "DELETE",
          }),
        });
  ...
  return (
    <div className="space-y-3 p-4 border border-destructive/50 rounded-lg bg-destructive/5">
      <p className="text-sm font-medium">Delete "{eventName}"?</p>

      {sessionCount > 0 && (
        <div className="p-2 bg-amber-50 border border-amber-200 rounded text-sm">
          <p className="text-amber-800">
            This event has {sessionCount} session{sessionCount > 1 ? "s" : ""} that will also be affected.
          </p>
        </div>
      )}
